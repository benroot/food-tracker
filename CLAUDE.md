...
**First slice, built**: a flat, reverse-chronological list covering the last 30 days, one row per day, showing that day's food-log entry count (meal events, i.e. `log_entries` rows — a 3-item breakfast counts as 1, not 3) and total calories consumed. Days with no logging still appear, as `0 entries, 0 cal`, rather than being skipped — gaps are meant to be visible (e.g. to spot a day you forgot to log), not hidden. Computed live via `app.py`'s `_daily_food_trends()`: one grouped SQL query over `log_entries`/`log_entry_items` for the 30-day window, then zero-filled in Python against the full 30-date list so missing days still render. No new tables, columns, or indexes — the existing schema is sufficient at this scale (a live `GROUP BY` over a single-user's data is instant either way). Weight, exercise calories burned, and net calories per day are not yet on this page — planned as later additions to the same `/trends` route rather than a separate page per data type.
*Usable for*: at this stage, a quick reverse-chronological read on food-logging volume and calorie totals day-by-day, and a way to notice logging gaps. The fuller trend/dashboard value (weight, exercise, net calories, selectable ranges) arrives in later iterations of this same page.
