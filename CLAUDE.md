...
- **`AUTH_PASSWORD_HASH`**: a Werkzeug `generate_password_hash` value using **`pbkdf2:sha256`, not the default `scrypt`** — production's Python 3.8.20 build on the cPanel host raises `AttributeError: module 'hashlib' has no attribute 'scrypt'` at verification time (scrypt needs OpenSSL 1.1+ built with scrypt support, which that host's Python doesn't have). This is a hard hosting-compatibility requirement, not a style preference — don't switch back to scrypt without first confirming production's Python actually supports it. `pbkdf2_hmac` has no such dependency (pure HMAC, always in hashlib).
  Also **base64-encoded before storage, not the raw `pbkdf2:sha256:...` string** — discovered separately: cPanel's Setup Python App env-var pipeline corrupts the raw hash on save, silently dropping whatever falls between the `$` delimiters (almost certainly shell-style `$variable` interpolation happening somewhere in how cPanel/Passenger inject the value) — the failure showed up as a hash that verified correctly locally but never matched in production, with the corrupted length even varying between attempts depending on the random salt's content. Base64's alphabet has no `$` (or any other shell metacharacter), so there's nothing left to mangle. `app.py` decodes it back to the real hash immediately after reading it from `os.environ`; `check_password_hash` and everything else downstream never knows the encoding step happened. Set via `.env` locally and cPanel's environment-variable UI in production — same treatment as `ANTHROPIC_API_KEY`.
...
- **`generate_password_hash.py`** (project root, run locally only — `python generate_password_hash.py`): prompts twice via `getpass` (never echoed, never touches shell history), hashes with Werkzeug's scrypt default, then base64-encodes the result before printing it — that printed value (not the raw hash) is what gets pasted into `.env` / cPanel's UI. Re-run any time the password should change; there's no in-app way to change it.
- **`verify_password_hash.py`** (project root, local diagnostic only): paste in an `AUTH_PASSWORD_HASH` value (base64, as it should appear in `.env`/cPanel) and a password to test, and it reports whether they actually match — mirrors the exact base64-decode + `check_password_hash` steps `app.py` performs, useful for isolating "is the stored value wrong" from "is the app logic wrong" without touching the live site.
...
### Phase 9 — Historical dashboard / trends (in progress, building iteratively)
A new `/trends` page summarizing trends over time rather than only ever showing "today." The eventual scope: weight over weeks/months, daily calorie counts, daily exercise calories burned, and net calories per day, across a selectable date range — still not fully designed, and being built one useful slice at a time rather than all at once. Chart vs. table, selectable ranges beyond the current fixed window, and how it relates to (or replaces) Phase 7's brief "history/trend views" mention remain open for later iterations.

**First slice, built, later replaced**: the original first slice was a flat, reverse-chronological list — one row per day, food-log entry count and total calories, zero-filled so days with no logging still showed as `0 entries, 0 cal` rather than being skipped. That list was fully replaced (not kept alongside) by the second slice below, once a chart-style view was wanted instead of a table-style one.

**Second slice, built**: two inline SVG sparklines — weight, then calories, in that order — covering the last 30 days, computed and rendered server-side with **no charting library and no CDN** (consistent with the no-silent-additions / minimal-stack rules: a sparkline's own math is small enough that a library would be net-negative complexity here). Key design points, since they're easy to get subtly wrong on a second pass:
- **Y-axis is tight-fit to each chart's own `[min, max]`, not zero-anchored.** Standard sparkline convention — a zero-anchored axis would flatten genuinely meaningful day-to-day weight/calorie variation into a nearly flat line. `app.py`'s `_sparkline_svg()` does this scaling generically for both charts.
- **X-axis is proportional to elapsed days from the window start, not list position.** This matters most for weight, which is deliberately *not* zero-filled the way calories are (`_weight_trend()` — weight logging is inherently sparse, and "0 lbs" isn't a meaningful placeholder for a day you didn't weigh in, unlike food's "0 cal"). A real gap between weight logs renders as a stretched flat/sloped segment rather than being visually compressed away, because each point's x-position comes from its actual date offset, not its index in the list of points.
- Calories still reuses the original `_daily_food_trends()` (zero-filled, today-first) unchanged — `_calorie_trend()` just reverses it to chronological order and re-indexes by day-offset for the shared sparkline builder.
- Each chart has a short visible caption (e.g. `181.5 lbs · range 181.5–185`, `900 cal today · range 0–2600`) rather than a separate hidden accessibility-only text block — the same numbers serve sighted users and screen readers at once, and the decorative `<svg>` itself is `aria-hidden`.
- No new tables, columns, or indexes — same reasoning as the first slice, this is still well within what a live query can compute instantly at single-user scale.

**Refined after the fact**: these aren't pure sparklines anymore — each chart got a minimal axis (a vertical + horizontal line, with min/max value labels on the y-axis and window start/end date labels on the x-axis), since a bare shape with no orientation at all turned out to be too little context. Still no gridlines or in-between tick marks, and still no charting library — `_sparkline_svg()` computes the axis line/label positions the same way it computes the polyline, just with real margins (`SPARKLINE_MARGIN_*`) reserved for the label text instead of a single uniform pad. Weight's y-axis also got a floor: `min_range` (`WEIGHT_MIN_AXIS_RANGE = 15`, i.e. 15 lbs) pads the axis symmetrically around the data's own midpoint whenever the actual span is tighter than that, so a stable weight (e.g. a real 230–235 lb range) doesn't get stretched to fill the full chart height and look more dramatic than it is — the axis shows the padded range (e.g. 225–240), while the caption below it still states the real measured min/max, so nothing about what actually happened is obscured by the padding. Calories has no such floor (its natural range is usually already wide) but the parameter is generic on `_sparkline_svg()` if that changes later.

Exercise calories burned and net calories per day are still not on this page — remain a planned future addition to the same `/trends` route.
*Usable for*: an at-a-glance shape of weight and calorie trends over the last month, including whether logging has gaps (a long flat/sloped weight segment, or a food day sitting at the very bottom of its own range, both read as "nothing logged that day"). The fuller trend/dashboard value (exercise, net calories, selectable ranges) arrives in later iterations of this same page.
